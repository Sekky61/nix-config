...
- Custom options are always put under `options.michal.*` with a config mirror under `config.michal.*`.
...
- Host config passes `username`, `hostname`, `inputs`, `self`, `lib` via `specialArgs` (origin is `hosts/default.nix`).

- Modules live under `modules/` or `services/` and are imported by `modules/default.nix` or `services/default.nix`.
- All new modules should be imported
...
See `modules/gui-packages/steam.nix` for example of a simple, conventional module.
...
- If you change structure of modules in a dramatic fashion, check `generate-docs.nix` so options show up in docs.
